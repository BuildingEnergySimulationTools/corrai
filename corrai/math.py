...
def aggregate_time_series(
    ...
    - reference_df (pd.DataFrame | pd.Series None, optional):
        reference one column DataFrame for error function aggregation.
        If provided, both result_df and reference_df should have
        the same first dimension. Default is None.
    ...
    if reference_df is not None:
        ...
        if not result_df.shape[0] == reference_df.shape[0]:
            raise ValueError(
        ...
        return pd.Series(
            [
                agg_method(
                    result_df.iloc[:, i], reference_df.iloc[:, 0], **agg_method_kwarg
                )
